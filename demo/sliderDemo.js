...
(function($, undefined){
	...
	$.widget("ui.sliderDemo", {
		options:{
			...
			max: 100,
			json: {},
			currentMin: (last_loaded_min_max['min']!= undefined)?last_loaded_min_max['min']:10,
			currentMax: (last_loaded_min_max['max']!= undefined)?last_loaded_min_max['max']:50,
		},
		...
		_createSlider: function(){
			...
			if(last_loaded_min_max['min'] != undefined) {
				this.options.currentMin = last_loaded_min_max['min'];
				this.options.currentMax = last_loaded_min_max['max'];
			}
			else {
				if(this.options.defaultMin && this.options.defaultMax) {
					this.options.currentMin = this.options.defaultMin;
					this.options.currentMax = this.options.defaultMax;
				}
				else {
					this.options.currentMin = this.options.json.range.min;
					this.options.currentMax = this.options.json.range.max;
				}
			}

			slider[this.options.name]({
				bounds: {min: this.options.json.range.min, max: this.options.json.range.max},
				defaultValues: {min: this.options.currentMin, max: this.options.currentMax}
			});
			this._elements.slider = slider;
			this._elements.slider.bind("valuesChanging valuesChanged userValuesChanged", $.proxy(this._updateMap, this));
		},

		_updateMap: function(e, data){
			var date_min, date_max;
			if(data == undefined) {
				if(this.options.currentMin != undefined) {
					date_min = this.options.currentMin;
					date_max = this.options.currentMax;
				}
				else if(this.options.defaultMin != undefined) {
					date_min = this.options.defaultMin;
					date_max = this.options.defaultMax;
				}
				else {
					alert('Slider default min max values are missing...');
				}
			}
			else {
				date_min = Math.round(data.values.min);
				date_max = Math.round(data.values.max);

				this.options.currentMin = last_loaded_min_max['min'] = Math.round(data.values.min);
				this.options.currentMax = last_loaded_min_max['max'] = Math.round(data.values.max);
			}

			// var range_data = this._getRangeData(date_min, date_max);
			// var markers = getMarkers(this.options.json.map, range_data, this.options.json.selected_type);
			// clearMarkers();
			// setAllMap(this.options.json.map, markers);

			// var range_markers = this._getRangeMarkers(this.options.json.markers,date_min,date_max);

			setMarkers(this.options.json.markers, date_min, date_max);
		},

		_getRangeMarkers: function(markers_map, min, max) {
			var range_markers = new Array;
			// console.log(markers_map)
			for(var date in markers_map) {
				if(min <= date && date <= max) {
					for(var i=0;i<markers_map[date].length; i++) {
						// if(isNaN(json[i].date_first_isolated) || (min <= json[i].date_first_isolated && json[i].date_first_isolated <= max)) {
						range_markers.push(markers_map[date][i]);
						// }
					}
				}
			}
